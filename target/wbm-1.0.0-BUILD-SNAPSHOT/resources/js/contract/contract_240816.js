...
//    	checkboxHandler("compltedList");
    ...
    function listContractInfo(pageIndex, isRefresh = 0) {
    	...
    	for(let keyValue of resultJson.contractData) {
			...
			resultData += '<td class="list_" data-purchase="o"><span class="purchase_icon purchase_info" sales_code=' + keyValue.tsi_code + '></span>';
			if(keyValue.tci_code) resultData += '<span class="purchase_icon"></span>';
			resultData += '</td>';
    ...
    function initPurchaseProductInfo() {
    	...
    	rootTag.find("#productPreview").children().remove();
    }
    
    function initPurchaseWorkInfo() {
    	let rootTag = modal.find("#workModal");
    	rootTag.find('input[type="text"]').val("");
    	rootTag.find("#workCompany").prop('selectedIndex', 0);
    ...
    $(document).on("click", ".purchase_info", function() {
    	let salesCode = $(this).attr("sales_code") || 0;
   		...
   		$("#bill_info_save").attr("sales_code", salesCode);
    	...
    	$("#workCompanyName").children("option:not(:first)").remove();
    	if(resultJson.workCompanyInfo) {
    		for(let keyValue of resultJson.workCompanyInfo) {
    			$("#workCompanyName").append('<option value="' + keyValue.twc_code + '">' + keyValue.twc_name + '</option>');
    		}
    	}
    ...
    $("#purchase_info_save").click(function() {
    	let salesCode = $(this).attr("sales_code");
    	if(salesCode <= 0) {
    		alert('계약정보를 선택하여 주십시요');
    		return;
    	}
        ...
        action_popup.confirm("입력하신 내용을 등록하시겠습니까?", function (res) {
        	if (res) {
	        	...
	        	formData.set("imgFolderName", "product");  
	        	if(purchaseFileInfo) {
	        		for(let i=0; i<purchaseFileInfo.length; i++) {
	        			for(let j=0; j<purchaseFileInfo[i].length; j++) {
	        				formData.append(`files[${i}][${j}]`, purchaseFileInfo[i][j]);
	        			}
	        		}	        		
	        	}
	            ...
	            if (resultJson == -99) {
	                ...
	                if (resultJson > 0) {
	                	...
	                	if(salesCode > 0) {
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(2).text(rootFormTag.find("#contractUserCate option:selected").text());
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(3).text(rootFormTag.find("#contractDemand").val());
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(4).text(rootFormTag.find("#contractOrder").val());
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(5).find("a").text(rootFormTag.find("#contractBusName").val());
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(6).find("div").html("<p>" + rootFormTag.find("#contractBusSTime").val() + '</p><span>~</span><p>' + rootFormTag.find("#contractBusETime").val() + "</p>");
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(7).find("div").html("<p>" + rootFormTag.find("#contractServiceSTime").val() + '</p><span>~</span><p>' + rootFormTag.find("#contractServiceETime").val() + "</p>");
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(8).text(rootFormTag.find("#contractDate").val());
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(9).text(rootFormTag.find("#contractPrice").val());
	                		list.find("#productList").find("tr").eq(parentTagIndex).find("td").eq(10).text(rootFormTag.find("#contractVat").val());
    ...
    function loadProductDetailInfo() {
    	let salesCode = $("#bill_info_save").attr("sales_code"); 
    	...
    	for(let keyValue of resultJson) {
	    	...
	    	resultData += '<td class=""><a href="#" class="product_detail_info" product_code=' + (keyValue.tppi_code || 0) + '>' + keyValue.tpi_name + '</a></td>';
    ...
    $(document).on("click", ".product_detail_info", function() {
        ...
        rootTag.find("#productCompanyOther").val($('#productCompanyInfo option[value="' + keyValue.detail_productdata.tpp_tpccode + '"]').text() || "");
        rootTag.find("#productPrice").val(keyValue.detail_productdata.tpp_price || "");
        rootTag.find("#productPriceVat").val(keyValue.detail_productdata.tpp_price_vat || "");
        rootTag.find("#productCount").val(keyValue.detail_productdata.tpp_count || 0);
        ...
        rootTag.find("#productNameOther").val($('#productNameOther option[value="' + keyValue.detail_productdata.tpp_tpicode + '"]').text() || "");
        rootTag.find("#productMemo").val(keyValue.detail_productdata.tpp_memo || "");
        
        if(resultJson.file_product_data) {
        	let index = 0;
        	let fileData = '';
        	for(let keyValue of resultJson.file_product_data) {
        		...
        		fileData += '<td><a href="#none" onclick="location.href=\'' + downloadPath + '/salesDownload?resFileName=' + keyValue.tfi_res_name + '&fileName=' + keyValue.tfi_file_name + '&imageFolder=product\'" class="file_download"></a></td></tr>';
        		rootTag.find("#productPreview").append(fileData);  
        		index ++;
        	}
        }
    });
    ...
    $("#btn_add_product, #btn_edit_product").click(function() {
    	...
    	if(saveType != 2) {
    		...
    		rootTag.find("#productList tr").eq(trIndex).find("td:eq(2)").text(rootTag.find("#productNameOther").val());
    	...
    	action_popup.confirm("현재 선택된 데이터 상태를 변경하시겠습니까?", function (res) {
            if (res) {
	            ...
	            if (!resultJson) {
	        		...
	        		$("#compltedList tbody tr").eq(trIndex).find("td:eq(8)").text($("#payContractReponseDate").val() || "");
	        		$("#compltedList tbody tr").eq(trIndex).find("td:eq(9)").text($("#contractEditorAdmin").html() || "");
    ...
    $(document).on("click", ".btn_modalList_del", function() {
        ...
        if (result) {
            ...
            for(let i=0; i<parentTr.length; i++) parentTr.eq(i).find("td:eq(0)").text(i+1);       	
        }
    });
	            ...
	            if (resultJson == -99) {
	                ...
	                if (resultJson > 0) {
	                	...
	                	if(salesCode > 0) {
	                		...
	                		list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(6).find("div").html("<p>" + rootFormTag.find("#salesBusSTime").val() + '</p><span>~</span><p>' + rootFormTag.find("#salesBusETime").val() + "</p>");
	                		list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(7).find("div").html("<p>" + rootFormTag.find("#salesServiceSTime").val() + '</p><span>~</span><p>' + rootFormTag.find("#salesServiceETime").val() + "</p>");
	            ...
	            if (resultJson == -99) {
	                ...
	                if (resultJson > 0) {
	                	...
	                	if(salesCode > 0) {
	                		...
	                		list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(6).find("div").html("<p>" + rootFormTag.find("#contractBusSTime").val() + '</p><span>~</span><p>' + rootFormTag.find("#contractBusETime").val() + "</p>");
	                		list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(7).find("div").html("<p>" + rootFormTag.find("#contractServiceSTime").val() + '</p><span>~</span><p>' + rootFormTag.find("#contractServiceETime").val() + "</p>");
...
const listInclud_TotalSumValue = () => {
    ...
    for (let i = 0; i < listNotInclud_R_Group.length; i++) {
        ...
        listNotInclud_R.push(Number(listNotInclud_R_item.text()));07     
    }
