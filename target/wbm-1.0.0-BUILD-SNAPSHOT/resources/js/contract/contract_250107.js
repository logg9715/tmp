...
$(document).ready(function () {
    ...
    function loadPayCompany() {
        ...
        $("#contractCompany").children("option:not(:first)").remove();
        ...
        for (let keyValue of resultJson) {
            ...
            $("#contractCompany").append('<option value="' + keyValue.tpc_code + '">' + keyValue.tpc_name + "</option>");
        }
    }
    ...
    function listContractInfo(pageIndex, isRefresh = 0) {
    	let rootTag = $("#form_search_contract");
    	
    	let sortInfo = [];
        rootTag.find('.sortListRow').each(function(index) {
        	let selectedValue = $(this).find('.selSortValue').attr("value");
        	sortInfo.push(selectedValue);
        });
        ...
        */
        let total_info = {contract_price:0, contract_vat:0};
        let resultJson = axCallFunction("listContractInfo", formData, false);
        if (!resultJson) return;
        ...
        for (let keyValue of resultJson.contractData) {
            ...
            resultData += '<td class="list_section">' + (keyValue.tpc_name ||"") + "</td>";
            ...
            total_info.contract_price += parseInt((keyValue.tsi_contract_price || 0));
            total_info.contract_vat += parseInt((keyValue.tsi_contract_vat.replace(",", "") || 0));
            // if (gLoginLevel >= MANAGERLEVEL) {
            if (keyValue.tsi_confirm_level > 0) {
            	if(keyValue.work_count > 0 || keyValue.purchase_count > 0) {
            		resultData += '<td><span class="purchase_icon purchase_info" sales_code=' + keyValue.tsi_code + ' data-purchase="o"></span></td>"';
            	} else {
            		resultData += '<td><span class="purchase_icon purchase_info" sales_code=' + keyValue.tsi_code + ' data-purchase="x"></span></td>';
            	}
			...
			*/
            resultData += '<td class="list_status" data-status="' + confirmLevel + '">' + confirmLevel + '</td>';
            /*
        ...
        if(total_info.contract_price > 0) $("#total_info").text("계약금액 총합(vat포함) : " + numberWithCommas(total_info.contract_price) + ", 계약금액 총합(vat별도) : " + numberWithCommas(total_info.contract_vat));
        ...
        action_popup.confirm("입력하신 내용을 등록하시겠습니까?", function (res) {
            if (res) {
                ...
                if (resultJson <= 0) {
                    ...
                    $("#list_contract_info tr").eq(trIndex).find("td:eq(12) span").attr("contract_code", resultJson);
                    $("#list_contract_info tr").eq(trIndex).find("td:eq(12) span").attr("data-purchase", "o");
    ...
    // 매입정보 삭제
    $("#purchase_info_del").click(function () {
    	let salesCode = $("#purchase_info_save").attr("sales_code") || 0;
    	if (salesCode <= 0) {
    		alert("계약정보를 선택하여 주십시요");
    		return;
    	}
    	action_popup.confirm("선택하신 내용을 삭제하시겠습니까?", function (res) {
    		if (res) {
    			...
    			let resultJson = axCallFunction("deletePurchaseInfo", sendData, false);
    			if (resultJson <= 0) {
    				if (resultJson == -99) {
    					changePopupMessage("로그인을 먼저 하여 주십시요", modal);
    					window.location.href = "login";
    ...
    function loadProductDetailInfo(salesCode) {
        ...
        for (let keyValue of resultJson) {
            ...
            resultData += '<td class="">' + (keyValue.tpp_company_name || "") + "</td>";
            resultData += '<td class=""><a href="#" class="product_detail_info" product_code=' + (keyValue.tpp_code || 0) + ">" + (keyValue.tpp_product_name || "") + "</a></td>";
    ...
    function loadWorkDetailInfo(salesCode) {
        ...
        for (let keyValue of resultJson) {
            ...
            resultData += '<td class="">' + (keyValue.tiw_company_name || "") + "</td>";
            resultData += '<td class=""><a href="#" class="work_detail_info" work_code=' + (keyValue.tiw_code || 0) + ">" + keyValue.tiw_work_name + "</a></td>";
    ...
    $("#btn_add_product, #btn_edit_product").click(function () {
        ...
        if (saveType != 2) {
            ...
            resultData += '<td class="">';
            resultData += '<a href="#" class="product_detail_info">' + rootTag.find("#productNameOther").val() + "</a>";
            resultData += "</td>";
            ...
            rootTag.find("#productList tr").each(function(index) {
    ...
    */
    ...
    contract_tabBtn.click(function () {
        ...
        if(index == 1) {
        	$("#contract_ins_price").text("입력된 계약금(vat포함) : " + numberWithCommas($("#contractPrice").val()));
        }
    ...
    function initContractReadyMoney() {
        ...
        rootTag.find("#payCheck").prop("checked", false);
    ...
    function initPayBill() {
        ...
        rootTag.find("input[name='payCheck']").prop("checked", false);
    ...
    top_content.find(".btn_modal_show").click(function () {
        ...
        if (clickedBtnId === "btn_business_N") {
            ...
            modal.find("#btn_info_nextstep").hide(); // 다음단계 버튼 숨기기
            modal.find("#btn_info_edit, #btn_info_del").hide(); // 수정, 삭제 버튼 숨기기
            modal.find("#btn_info_save").show(); // 수정, 삭제 버튼 숨기기
    ...
    function salesLevelChange(tagInfo, levelType) {
        ...
        if (levelType < 0) {
        	...
        	if(confirmLevel >= BEFCONTRACT_CONFIRM_LEVEL && $("#contractDate").attr("contract_date") == "") {
        		alert("계약일자가 저장되지 않았습니다.");
        		return;
        	} else confirmLevel++;
        }
...
//                    list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td:eq(14)").find("div").text(SALES_LEVEL_NAME[confirmLevel]);
    ...
    $(".btn_nextStep, .btn_prevStep").click(function () {
        ...
        let is_nextstep = $(this).val() == 1?true:false;
        let confirmLevels = [];
        let checkVal = $(this).val();
        let checkedItems = $(".mainListCheckBox:checked");
        
        let over_confirm = false;
        let contract_date_none = false;
        checkedItems.each(function () {
        	let confirm_level = $(this).closest("tr").find("a").attr("confirm_level");
        	// console.log("confirm_level=", confirm_level, gLoginLevel);
        	if(is_nextstep) {
	        	if(gLoginLevel <= COMMONLEVEL && confirm_level > SALES_CONFIRM_LEVEL) {
	        		over_confirm = true;
	        		return;
	        	} else if(gLoginLevel <= MANAGERLEVEL && confirm_level > BEFCONTRACT_CONFIRM_LEVEL) {
	        		over_confirm = true;
	        		return;
	        	} else if(gLoginLevel > MANAGERLEVEL && confirm_level >= FINISHCONTRACT_CONFIRM_LEVEL) {
	        		over_confirm = true;
	        		return;
	        	}
	        	
	        	if(gLoginLevel <= MANAGERLEVEL && confirm_level == BEFCONTRACT_CONFIRM_LEVEL
	        	   && !$(this).closest("tr").find("td").eq(9).text()) {
	        		contract_date_none = true;
	        		return;
	        	} 
        	} else {
	        	if(gLoginLevel <= MANAGERLEVEL && confirm_level > BEFCONTRACT_CONFIRM_LEVEL) {
	        		over_confirm = true;
	        		return;
	        	} else if(gLoginLevel > MANAGERLEVEL && (confirm_level > FINISHCONTRACT_CONFIRM_LEVEL && confirm_level < AFTCONTRACT_CONFIRM_LEVEL)) {
	        		over_confirm = true;
	        		return;
	        	}
        	}
        });
        if(contract_date_none) {
        	alert("선택된 데이터 중 계약일자가 입력되지 않은 데이터들이 존재합니다.");
        	return;
        }
        if(over_confirm) {
        	alert("선택된 데이터 중 현 상태가 다음단계로 이동할 수 없는 데이터들이 존재합니다.");
        	return;
        }
        ...
        checkedItems.each(function () {
            ...
            over_confirm = false;
            if(is_nextstep) {
	        	if(gLoginLevel <= COMMONLEVEL && currConfirmLevel > SALES_CONFIRM_LEVEL) over_confirm = true;
	        	else if(gLoginLevel <= MANAGERLEVEL && currConfirmLevel > BEFCONTRACT_CONFIRM_LEVEL) over_confirm = true;
	        	else if(gLoginLevel > MANAGERLEVEL && currConfirmLevel >= FINISHCONTRACT_CONFIRM_LEVEL) over_confirm = true;
            } else {
            	if(gLoginLevel <= MANAGERLEVEL && currConfirmLevel > BEFCONTRACT_CONFIRM_LEVEL) over_confirm = true;
	        	else if(gLoginLevel > MANAGERLEVEL && (currConfirmLevel > FINISHCONTRACT_CONFIRM_LEVEL && currConfirmLevel < AFTCONTRACT_CONFIRM_LEVEL)) over_confirm = true;
            }	            
            if (lastConfirmLevel >= 0 && !over_confirm) {
                confirmLevelData.confirmLevel = lastConfirmLevel;
                confirmLevelData.salesCode = salesCode;
                confirmLevels.push(confirmLevelData);
            }
        });
        console.log("confirmLevels=", confirmLevels);
        if(confirmLevels.length <= 0) {
        	alert("단계를 변경할 데이터가 존재하지 않습니다.");
        	return;
        }
    ...
    function detailContractInfo(salesCode, self) {
        ...
        // 계약정보 출력
        rootTag.find("#contractCompany").val(resultJson.detail_data.tsi_tpccode).prop("selected", true);
        let idx = rootTag.find("#contractCompany option").index(rootTag.find("#contractCompany option:selected"));
        if (idx < 0) rootTag.find("#contractCompany option").eq(0).prop("selected", true);
        rootTag.find("#contractUserCate").val(resultJson.detail_data.tsi_tstcode).prop("selected", true);
        idx = rootTag.find("#contractUserCate option").index(rootTag.find("#contractUserCate option:selected"));
        ...
        rootTag.find("#contractDate").attr("contract_date", (resultJson.detail_data.tsi_contract_date||""));
        ...
        if (gLoginLevel > MANAGERLEVEL) {
            ...
            if (resultJson.detail_bill_publication) {
                ...
                resultJson.detail_bill_publication.tbp_pay_check == 1 ? rootBillTag.find("#payCheck").prop("checked", true) : rootBillTag.find("#payCheck").prop("checked", false);
        ...
        if (resultJson.detail_contract_readymoney) {
            ...
            for (let keyValue of resultJson.detail_contract_readymoney) {
                ...
                let pay_check = (keyValue.tpi_pay_check || 0) == 1?"검수완료":"검수전";
                resultData += '<td class="payCheck" pay_check=' + (keyValue.tpi_pay_check || 0) + "><span class='icon'></span>" + pay_check + "</td>";
                let bill_type = (keyValue.tpi_pay_type || 0) == 1?"발행완료":"발행전";
                resultData += '<td class="billType" bill_type=' + (keyValue.tpi_pay_type || 0) + "><span class='icon'></span>" + bill_type + "</td>";
                ...
                let pay_collection = (keyValue.tpi_collection || 0) == 1?"수금완료":"수금전";
                resultData += '<td class="payCollect" pay_collect=' + (keyValue.tpi_collection || 0) + "> <span class='icon'></span>" + pay_collection + "</td>";
                resultData += '<td class="">';
                resultData += '<button type="button" class="btn_modalList_del"></button>';
                resultData += "</td>";
                resultData += "</tr>";
    ...
    $(document).on("click", ".contractPayType", function () {
        ...
        if (($(this).closest("tr").find("td:eq(7)").attr("pay_check") || 0) == 1) rootFormTag.find("#payCheck").prop("checked", true);
        else rootFormTag.find("#payCheck").prop("checked", false);
        if (($(this).closest("tr").find("td:eq(8)").attr("bill_type") || 0) == 1) rootFormTag.find("#billType").prop("checked", true);
        else rootFormTag.find("#billType").prop("checked", false);
        rootFormTag.find("#payContractReponseDate").val($(this).closest("tr").find("td:eq(9)").text());
        rootFormTag.find("#adminMemo").val($(this).closest("tr").find("td:eq(10) span").attr("memo") || "");
        if (($(this).closest("tr").find("td:eq(11)").attr("pay_collect") || 0) == 1) rootFormTag.find("#payCollect").prop("checked", true);
    ...
    $("#addContractInfo, #editContractInfo").click(function () {
        ...
        if (gLoginLevel > MANAGERLEVEL) {
            ...
            */
            ...
            payCheck = rootTag.find("#payCheck").is(":checked") ? 1 : 0;
        ...
        let pay_check = (payCheck || 0) == 1?"검수완료":"검수전";
        let bill_type = (billType || 0) == 1?"발행완료":"발행전";
        let pay_collection = (payCollect || 0) == 1?"수금완료":"수금전";
        if (saveType != 2) {
            let resultData = '<tr class="list_row">';
            ...
            if (gLoginLevel > MANAGERLEVEL) {
                ...
                resultData += '<td class="payCheck" pay_check=' + payCheck + "><span class='icon'></span>" + pay_check + "</td>";
                resultData += '<td class="billType" bill_type=' + billType + "><span class='icon'></span>" + bill_type + "</td>";
                ...
                // 수금수정
                resultData += "<td pay_collect=" + payCollect + ' class="payCollect"><span class="icon"></span>' + pay_collection + '</td>"';
            } else {
                // resultData += '<td bill_type=0 class="">발행전</td>';
            	resultData += "<td class='payCheck' pay_check='0'><span class='icon'></span>" + pay_check + "</td>";
                resultData += "<td class='billType' bill_type='0'><span class='icon'></span>" + bill_type + "</td>";
                resultData += '<td class=""></td>';
                resultData += '<td class=""><span class="icon" memo_code="0" memo=""></span></td>'; //메모
                resultData += '<td class="" pay_collect=0><span class="icon"></span>' + pay_collection + '</td>';
            }
            resultData += '<td class="">';
            resultData += '<button type="button" class="btn_modalList_del"></button>';
            resultData += "</td>";
            ...
            if (gLoginLevel > MANAGERLEVEL) {
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(7)").text(pay_check);
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(7)").attr("pay_check", payCheck);
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(8)").text(bill_type);
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(8)").attr("bill_type", billType);
                // rootTag
                //     .find("#compltedList tbody tr")
                //     .eq(trIndex)
                //     .find("td:eq(7)")
                //     .text(billType == 0 ? "발행전" : "발행완료");
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(9)").text(publicationDate);
                rootTag
                    .find("#compltedList tbody tr")
                    .eq(trIndex)
                    .find("td:eq(10) span")
                    .attr("memo_code", memoVal === "" ? 0 : 1);
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(10) span").attr("memo", memoVal);
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(11)").text(pay_collection);
                rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(11)").attr("pay_collect", payCollect);
                // rootTag
                //     .find("#compltedList tbody tr")
                //     .eq(trIndex)
                //     .find("td:eq(10)")
                ...
                if (resultJson == -99) {
                    ...
                    if (resultJson > 0) {
                        ...
                        if (salesCode > 0) {
                            ...
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(3).text(rootFormTag.find("#salesUserCate option:selected").text());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(4).text(rootFormTag.find("#salesDemand").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(5).text(rootFormTag.find("#salesOrder").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(6).find("a").text(rootFormTag.find("#salesBusName").val());
                            list.find("#list_contract_info")
                                ...
                                .eq(7)
                                .find("div")
                                .html("<p>" + rootFormTag.find("#salesBusSTime").val() + "<p>" + rootFormTag.find("#salesBusETime").val() + "</p>");
                            list.find("#list_contract_info")
                                .find("tr")
                                .eq(parentTagIndex)
                                .find("td")
                                .eq(8)
                                .find("div")
                                .html("<p>" + rootFormTag.find("#salesServiceSTime").val()  + "<p>" +  rootFormTag.find("#salesServiceETime").val() + "</p>");
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(10).text(numberWithCommas(rootFormTag.find("#salesContractPrice").val()));
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(11).text(numberWithCommas(rootFormTag.find("#salesContractVat").val()));
    ...
    modal.find("#contract_info_save, #contract_info_edit").click(function () {
    	...
    	$("#compltedList tbody tr").each(function () {
    		total_price += parseInt(($(this).find("td").eq(3).text() || "0").replace(",", ""));
    	});
    	let contract_price = parseInt(($("#contractPrice").val() || "0").replace(",", ""));
    	if(gLoginLevel <= MANAGERLEVEL && total_price != 0 && total_price >= contract_price) {
    		alert("계산서 발행이 완료되어 수정할 수 없습니다.");
    		return;
    	}
            ...
            if (res) {
                ...
                if (resultJson == -99) {
                    ...
                    if (resultJson > 0) {
                        changePopupMessage("처리가 완료되었습니다.", modal);
                        if (salesCode > 0) {
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(3).text(rootFormTag.find("#contractUserCate option:selected").text());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(4).text(rootFormTag.find("#contractDemand").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(5).text(rootFormTag.find("#contractOrder").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(6).find("a").text(rootFormTag.find("#contractBusName").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(6).find("a").attr("confirm_level", confirmLevel);
                            list.find("#list_contract_info")
                                .find("tr")
                                .eq(parentTagIndex)
                                .find("td")
                            ...
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(9).text(rootFormTag.find("#contractDate").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(10).text(rootFormTag.find("#contractPrice").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td").eq(11).text(rootFormTag.find("#contractVat").val());
                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td:eq(15)").attr("data-status", SALES_LEVEL_NAME[confirmLevel]);
                            //                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td:eq(14)").find("div").text(SALES_LEVEL_NAME[confirmLevel]);
                        } else listContractInfo(1, 1);
                    } else {
                        changePopupMessage("처리 중 오류가 발생하였습니다.", modal);
