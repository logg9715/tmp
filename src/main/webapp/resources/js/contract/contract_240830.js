...
$(document).ready(function () {
    ...
    function listContractInfo(pageIndex, isRefresh = 0) {
        ...
        for (let keyValue of resultJson.contractData) {
            ...
            resultData += "<p>" + keyValue.tsi_bus_starttm + "</p><span>~</span>";
            ...
            resultData += '<td class="list_term_C">';
            resultData += '<div class="date_wrap">';
            resultData += "<p>" + keyValue.tsi_as_starttm + "</p><span>~</span>";
            // resultData += '<span>~</span>';
            ...
            if(gLoginLevel >= MANAGERLEVEL) resultData += '<td class="list_" data-purchase="o"><span class="purchase_icon purchase_info" sales_code=' + keyValue.tsi_code + " contract_code=" + (keyValue.tci_code || 0) + "></span></td>";
        ...
        action_popup.confirm("입력하신 내용을 등록하시겠습니까?", function (res) {
            if (res) {
                ...
                if (resultJson == -99) {
                	...
                	$("#list_contract_info").eq(trIndex).find("td:eq(11) span").attr("contract_code", resultJson);
    ...
    $("#btn_add_product, #btn_edit_product").click(function () {
        ...
        let memoVal = $('#productMemo').val().trim() === "" ? 0 : 1;
        let saveType = 1;
        ...
        if (saveType != 2) {
            ...
            resultData += '<td class=""><span class="icon" memo_code="'+ memoVal +'"></span></td>';
            resultData += '<td class="">';
    ...
    */
    ...
    modal.find(".btn_nextStep, .btn_prevStep").click(function () {
        ...
        let level = (modal.find(".btn_nextStep").attr("level") || 0);
    ...
    $("#addContractInfo, #editContractInfo").click(function () {
        ...
        if (saveType != 2) {
            ...
            if (gLoginLevel > MANAGERLEVEL) {
            	...
            	rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(7)").text(billType == 0?"발행전":"발행완료");
            	rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(8)").text(publicationDate);
            	rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(9) span").attr("memo_code", memoVal === "" ? 0 : 1);
            	...
            	rootTag.find("#compltedList tbody tr").eq(trIndex).find("td:eq(10)").text(payCollect == 0?"수금전":"수금완료");
            }
        }
    ...
    modal.find("#contract_info_save, #contract_info_edit").click(function () {
        ...
        let confirmLevel = gLoginLevel > MANAGERLEVEL ? 2 : modal.find("#contract_info_edit").attr("confirm_level");
