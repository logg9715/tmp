...
//            rootTag.find("#productList tr").eq(trIndex).find("td:eq(6)").text(rootTag.find("#productMemo").val());
            ...
            rootTag.find("#productList tr").eq(trIndex).find("td:eq(7) span").attr("memo_code", memoVal === "" ? 0 : 1);
        }
        /*
    ...
    $("#btn_add_work, #btn_edit_work").click(function () {
        ...
        if (saveType != 2) {
            ...
            rootTag.find("#workInfoList tr").eq(trIndex).find("td:eq(7) span").attr("memo_code", memoVal === "" ? 0 : 1);
        }
        /*
    ...
    */
    ...
    $(document).on("click", ".btn_info_change", function () {
        ...
        if (confirmLevel == 0) {
            ...
            // modal.find("#btn_info_prevstep").hide();
            ...
            modal.find("#contract_info_save").show();
            modal.find("#contract_info_nextstep, #contract_info_prevstep").show();
            modal.find("#contract_info_edit, #contract_info_del").show();
...
//                            list.find("#list_contract_info").find("tr").eq(parentTagIndex).find("td:eq(14)").find("div").text(SALES_LEVEL_NAME[confirmLevel]);
    ...
    $("#btn_auto_calculate_rate").click(function() {
        ...
        if ((!isNumeric.test(priceValue) && priceValue !== '') || 
        	(!isNumeric.test(vatValue) && vatValue !== '') ||
        	(!isNumeric.test(vatRateValue) && vatRateValue !== '')) {
            alert('숫자만 입력해 주세요.');
            return;
        }
